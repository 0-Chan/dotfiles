...
자격증명은 아래 둘 중 하나로 설정 (둘 다 `default` 프로파일로 잡혀야 30번 스크립트가 인식).

**A. 정적 키 — `aws configure`**
...
- 한 번 설정하면 **만료 없이** 계속 동작 (편함)
- 평문 key+secret이 `~/.aws/credentials`에 저장됨 → 유출 시 수동 회전 전까지 항구적 위험
- 추가 login 단계 없음. configure가 끝나면 그 자체로 자격증명 완성

**B. SSO — `aws configure sso` + `aws sso login`**

```bash
aws configure sso     # 인터랙티브: start URL, account, role, CLI default profile name=default
aws sso login          # 브라우저 사인-인 → ~/.aws/sso/cache 에 단기 토큰 캐시
```

- 토큰이 **8–12h** (org 정책) 후 만료 → 만료 시 `aws sso login` 한 번 더
...
>
> IAM 권한 (정적 키의 IAM user 또는 SSO permission set): `secretsmanager:GetSecretValue`, Region: `ap-northeast-2`
